...
const UI = {
    ...
    renderTabs() {
        ...
        container.innerHTML = this.openTabs.map(tab => {
            ...
            return `
                <div class="tab ${tab === this.currentPanel ? 'active' : ''}" data-tab="${tab}" onclick="UI.navigateTo('${tab}')">
        ...
        switch (this.currentPanel) {
            case 'dashboard':
                ...
                container.innerHTML = `
                    <div class="sidebar-section">
                        ...
                        <div class="sidebar-item" onclick="UI.navigateTo('vehicles')">
                            <i class="fas fa-car" style="color:var(--accent-blue)"></i> All Vehicles
                            <span class="item-badge badge-normal">${stats.total}</span>
                        </div>
                        <div class="sidebar-item" onclick="UI.navigateTo('vehicles')">
                            <i class="fas fa-check-circle" style="color:var(--status-normal)"></i> Normal
                            <span class="item-badge badge-normal">${stats.normal}</span>
                        </div>
                        <div class="sidebar-item" onclick="UI.navigateTo('vehicles')">
                            <i class="fas fa-exclamation-triangle" style="color:var(--status-warning)"></i> Warning
                            <span class="item-badge badge-warning">${stats.warning}</span>
                        </div>
                        <div class="sidebar-item" onclick="UI.navigateTo('vehicles')">
                            <i class="fas fa-times-circle" style="color:var(--status-exceeded)"></i> Exceeded
                            <span class="item-badge badge-exceeded">${stats.exceeded}</span>
                        </div>
                    </div>
                    <div class="sidebar-section">
            ...
            case 'mileage':
                container.innerHTML = `
                    ...
                    return `
                                <div class="sidebar-item" onclick="VehicleManager.showDetails('${v.id}')">
                                    <i class="fas ${VehicleManager.getTypeIcon(v.type)}" style="color:var(--status-${status})"></i>
                                    ${v.id}
                                    <span class="item-badge badge-${status}">${(v.mileage || 0)}</span>
                                </div>`;
                }).join('')}
                    </div>`;
                break;
                ...
                container.innerHTML = `
                    <div class="sidebar-section">
                        ...
                        <div class="sidebar-item">
                            <i class="fas fa-bell" style="color:var(--accent-blue)"></i> Total Alerts
                            <span class="item-badge badge-normal">${alerts.length}</span>
                        </div>
                        <div class="sidebar-item">
                            <i class="fas fa-envelope" style="color:var(--status-warning)"></i> Unread
                            <span class="item-badge badge-warning">${unread}</span>
                        </div>
                        <div class="sidebar-item">
                            <i class="fas fa-exclamation-triangle" style="color:var(--status-warning)"></i> Warnings
                            <span class="item-badge badge-warning">${alerts.filter(a => a.type === 'warning').length}</span>
                        </div>
                        <div class="sidebar-item">
                            <i class="fas fa-times-circle" style="color:var(--status-exceeded)"></i> Critical
                            <span class="item-badge badge-exceeded">${alerts.filter(a => a.type === 'critical').length}</span>
                        </div>
                    </div>`;
                break;
    ...
    updateStatusBar() {
        ...
        const stats = DataStore.getStats();
        const unread = DataStore.getUnreadAlertCount();
    ...
    updateAlertBadge() {
        const count = DataStore.getUnreadAlertCount();
    ...
    updateClock() {
        ...
        document.getElementById('status-time').innerHTML =
            `<i class="fas fa-clock"></i> ${now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`;
    },
...
const DriverView = {
    render() {
        ...
        container.innerHTML = vehicles.map(v => {
            ...
            return `
                <div class="vehicle-card-detail" style="margin-bottom:20px;">
                    ...
                    <div class="vehicle-card-body">
                        <div class="vehicle-info-grid">
                            ...
                            <div class="vehicle-info-item">
                                ...
                                <span>${(v.mileage || 0).toLocaleString()} miles</span>
                            </div>
                            <div class="vehicle-info-item">
                                <label>Remaining</label>
                                <span style="color:var(--status-${status})">${remaining > 0 ? remaining.toLocaleString() + ' miles' : 'EXCEEDED'}</span>
                            </div>
                            <div class="vehicle-info-item">
                        ...
                        <div style="margin-top:16px; display:flex; gap:8px;">
                            <button class="btn btn-primary" onclick="MileageManager.openMileageModal('${v.id}')">
                                <i class="fas fa-road"></i> Log Mileage
                            </button>
                            <button class="btn btn-secondary" onclick="VehicleManager.showDetails('${v.id}')">
                                <i class="fas fa-eye"></i> View Details
                            </button>
                        </div>
                        ${logs.length > 0 ? `
                            <div style="margin-top:20px;">
                                <h4 style="color:var(--text-bright); font-size:14px; margin-bottom:10px;"><i class="fas fa-history" style="color:var(--accent-blue)"></i> Recent Entries</h4>
                                <div class="table-container">
                                    <table class="data-table">
                                        <thead><tr><th>Date</th><th>From</th><th>To</th><th>Added</th></tr></thead>
                                        <tbody>
                                            ${logs.map(l => `
                                                <tr>
                                                    ...
                                                    <td style="color:var(--accent-green)">+${l.milesAdded.toLocaleString()}</td>
                                                </tr>`).join('')}
                                        </tbody>
                                    </table>
                                </div>
